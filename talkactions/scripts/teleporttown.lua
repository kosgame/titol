function onSay(cid, words, param, channel)
	...
	if(t[(master and 1 or 2)]) then
		...
		if(not tid or (isPlayerGhost(tid) and getPlayerAccess(tid) > getPlayerAccess(cid))) then
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_RED, "Player " .. t[(master and 1 or 2)] .. " not found.")
		end
	end
	...
	if(type(pos) ~= 'table' or isInArray({pos.x, pos.y}, 0)) then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_RED, "Town " .. t[1] .. " does not exists or has invalid temple position.")
	end
