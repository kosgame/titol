...
function creatureSayCallback(cid, type, msg)
	if(not npcHandler:isFocused(cid)) then
		return false
			...
			if msgcontains(msg, 'red lantern') then
			if getPlayerItemCount(cid,11200) >= 1 then
				selfSay('Did you bring me the red lantern?', cid)
				talk_state = 1
			else
				selfSay('I need a {red lantern}, to give you the Dragon Statue. Come back when you have them.', cid)
				talk_state = 0
			end
			...
			talk_state = 0
			if getPlayerItemCount(cid,11200) >= 1 then
			if doPlayerRemoveItem(cid,11200, 1) == TRUE then
							selfSay(done, cid)
